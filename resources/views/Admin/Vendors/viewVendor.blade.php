           ...
           <div class="col-12 ">
               <div class="card">                    
                   <div class="card-body">
                       <div class="row">
                           <div class="col-6">
                               <h3 class="header-title">Vendors</h3>
                           </div>
                           <div class="col-6">
                               <div class="" style="float:right;margin-top:-10px;">                                
                                   <a href="addVendor" class="btn btn-primary waves-effect waves-light" >
                                    <i class="mdi mdi-plus-circle mr-1" ></i> Add Vendor</a>
                               </div>
                           </div>
                       </div>
                       <div class="row">
                       ...
                       <table id="key-datatable" class="table dt-responsive nowrap">
                           ...
                           <tbody>
                               ...
                               <tr>
                                   ...
                                   <td>{{$vendor->gstNo}}</td>
                                   <?php 
                                    $replaced = Str::replace('/', ', ', $vendor->address);
                                   ?>
                                   <td>{{$replaced}}</td>
                                   <td><i style='color:#5f82bd;font-size:20px;'  class='fa fa-edit' onclick="updateModal({{$vendor->id}})"></i>&emsp;&emsp;&emsp;<i  style='color:red;font-size:20px;' class='fa fa-trash' id="sa-warning" onclick='deleteVendor("{{$vendor->id}}")'></i>&emsp;&emsp;&emsp;<?php if($vendor['status'] === 'SUSPENDED') { ?><i  style='color:blue;font-size:20px;' class='fa fa-eye-slash' onclick='changeStatus("{{$vendor->id}}")'></i><?php } else {?><i  style='color:blue;font-size:20px;' class='fa fa-eye' onclick='changeStatus("{{$vendor->id}}")'></i><?php }?></td>
                               </tr>
                               @endforeach
